...
class Icon {
	constructor(type: string, public icon: string, public color: string = null, public hint: string | null = null) { this.icon = type + ' ' + icon;}
}
...
class Action {
	constructor(public icon: string, public label: string, public onClick: (item) => void) {}
}
...
export default class ListCard{
	...
	static Cardify(
		...
		actions: Array<Action | null> = null
	...
	static Action (icon: string, label: string, onClick: (item) => void) { return new Action(icon, label, onClick);}
}